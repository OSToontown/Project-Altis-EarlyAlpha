...
while True:
  try:
    ...
    safeHarbor = {'Raging River' 'Sunshine Ridge'}
    superDistricts = {'Wacky Falls'}
      ...
      for skey in shards:
        ...
        if shard['invasion'] == None:
          if shard['name'] in superDistricts:
            typ = int(1)
            suit = int(4)
            client.startInvasion(generate_token(700), int(skey), 1, 4, 2)
            count = count + 1
        ...
        for skey in shards:
          ...
          if shard['invasion'] == None and not shard['name'] in safeHarbor:
            ...
            if r < BaseInvasionChance and not shard['name'] in superDistricts:
              ...
              client.startInvasion(generate_token(700), int(skey), typ, suit, 1, 1)
              print 'Calling invasion for %s with %d,%d'%(shard['name'],typ,suit)
      print "tick..(was %d)\n"%(count)
      time.sleep(60)
  except Exception, e:
