...
def dsystem(message):
    """
    Broadcast a delayed <message> to the game server.
    """
    t = message.split(' ',1)
    if len(t) < 2:
        return "Need at least two arguments, a time and a string"

    when = t[0].split(':', 2);
    if len(when) == 0 or len(when) > 3:
        return "Need a time either as hour:min:sec, min:sec, or sec"

    try:
        if len(when) == 3:
            next = (int(when[0]) * 3600) + (int(when[1]) * 60) + int(when[2])
        else:
            if len(when) == 2:
                next = (int(when[0]) * 60) + int(when[1])
            else:
                next = int(when[0])
    except:
        return "Need a time either as hour:min:sec, min:sec, or sec"
    
    taskMgr.doMethodLater(next, system, 'maintenance-task',
                          extraArgs=[t[1]])

    return "sending message %  into the future" % (next)
...
def stopMain():
    taskMgr.remove('maintenance-task')
    system('Project Altis is not going down for maintenance! Stay Tooned!')
    return 'Stopped the maintenance countdown'
