...
class DistributedFindFour(DistributedNode.DistributedNode):
    ...
    def moveCameraForGame(self):
        ...
        if self.seatPos <= 2:
            ...
            interv = LerpPosHprInterval(base.camera, 2, position, Vec3(0, -38, 0), base.camera.getPos(), base.camera.getHpr())
        else:
            position = self.table.seats[4].getPos()
            position = position + Vec3(0, 12.8, 8)
            if base.camera.getH() < 0:
                interv = LerpPosHprInterval(base.camera, 2, position, Vec3(-180, -20, 0), base.camera.getPos(), base.camera.getHpr())
            else:
                interv = LerpPosHprInterval(base.camera, 2, position, Vec3(180, -20, 0), base.camera.getPos(), base.camera.getHpr())
        if interv:
            interv.start()
    ...
    def enableTurnScreenText(self, player):
        ...
        if player == self.playerNum:
            ...
            message2 = "It's Red's Turn"
            color = (1, 0, 0, 1)
        elif player == 2:
            message2 = "It's Yellow's Turn"
            color = (1, 1, 0, 1)
    ...
    def setGameState(self, tableState, moveCol, movePos, turn):
        ...
        if didIWin != None:
            base.localAvatar.setSystemMessage(2, "Congratulations! You've won a game of Find Four!")
