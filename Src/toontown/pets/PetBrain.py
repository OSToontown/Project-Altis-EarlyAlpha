...
class PetBrain(DirectObject.DirectObject):
    ...
    def _handlePhraseObserve(self, observe):
        ...
        def _handleDoTrick(trickId, avId, self = self):
            avatar = simbase.air.doId2do.get(avId)
            if avatar:
                if self.isAttendingUs(self.pet, avatar) and not self.attendingAv:
                    if not self.goalMgr.hasTrickGoal():
                        self._handleLookingAtOtherStart(avatar.doId)
                        self.attendingAv = avatar.doId
                    ...
                    self._handleLookingAtOtherStop(avatar.doId)
            elif avatar.doId == self.attendingAv:
                self.attendingAv = None
