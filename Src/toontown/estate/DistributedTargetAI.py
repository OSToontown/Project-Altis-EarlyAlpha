...
class DistributedTargetAI(DistributedObjectAI):
    ...
    def __init__(self, air):
        ...
        self.power = 0
        self.scoreDict = {}
        self.position = PinballCannonBumperInitialPos
    ...
    def __startNewGame(self, task):
        self.enabled = 1
        self.b_setPosition(randint(-50, 50), randint(20, 70), randint(35, 55))
        self.__newGame()
        self.d_updateState()
        taskMgr.doMethodLater(self.time, self.__gameOver, self.taskName('gameover'))
        return task.done
    ...
    def setCurPinballScore(self, avId, score, bonus):
        ...
        self.scoreDict[avId] = score * bonus
        if self.scoreDict[avId] > self.highscore:
            self.highscore = self.scoreDict[avId]
            self.d_updateHighscore(av, self.scoreDict[avId])
