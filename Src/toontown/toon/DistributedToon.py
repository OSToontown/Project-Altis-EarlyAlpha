...
class DistributedToon(DistributedPlayer.DistributedPlayer, Toon.Toon, DistributedSmoothNode.DistributedSmoothNode, DelayDeletable):
    ...
    def setTalk(self, fromAV, fromAC, avatarName, chat, mods, flags):
        ...
        if fromAV in self.ignoreList:
            self.d_setWhisperIgnored(fromAV)
            self.d_setWhisperIgnored(self.doId)
        ...
        if base.config.GetBool('want-sleep-reply-on-regular-chat', 0):
            if base.localAvatar.sleepFlag == 1:
                base.cr.ttFriendsManager.d_sleepAutoReply(fromAV)
    ...
    def warnLocalToon(self, reason):
        reason = 'Ruff oh! It seems like some representatives of the Toon Council have caught you misbehaving for %s.' % reason
        base.localAvatar.disableAvatarControls()
        self.confirm = TTDialog.TTGlobalDialog(doneEvent='confirmDone', message=reason, style=TTDialog.Acknowledge)
        self.confirm.show()
...
def sleep():
    if not base.localAvatar.neverSleep:
        ...
        return "Sleeping has been disabled for this session."
    else:
        base.localAvatar.enableSleeping()
        return "Sleeping has been enabled for this session."

@magicWord(category=CATEGORY_MODERATOR)
def res(msg):
    base.localAvatar.d_reqSCResistance(int(msg))
...
def mute(minutes):
    ...
    target.setSystemMessage(0, 'You have been muted for %s minutes. Behave next time!' % minutes)
    return 'Muted %s!' % target.getName()
...
def unmute():
    ...
    target.setSystemMessage(0, 'You have been unmuted by a staff member. Behave next time!')
    return 'Unmuted %s.' % target.getName()
