...
class DistributedToonAI(DistributedPlayerAI.DistributedPlayerAI, DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI):
    ...
    def announceGenerate(self):
        ...
        """if 'alphacodehehehehe' not in self.redeemedCodes and avId >= 100000000 and avId <= 100000055:
                                    simbase.air.codeRedemptionManager.redeemCode('alphacodehehehehe')
                                    av.d_setSystemMessage(0, 'Thank you for testing Project Altis! You have recieved a special alpha shirt and some jellybeans in the mailbox!')
                                else:
                                    pass"""
    ...
    def reqCogSummons(self, type, suitIndex):
        if type not in ('building', 'invasion', 'cogdo'):
    ...
    def assignNewCogSummons(self, level = None, summonType = None, deptIndex = None):
        ...
        summonTypes = ['building', 'invasion', 'cogdo']
...
@magicWord(category=CATEGORY_HEAD_MODERATION, types=[int, str])
def sos(count, name):
    """
...
def unites(value=32767):
    ...
    """
    if spellbook.getInvokerAccess() > CATEGORY_PROGRAMMER:
      av = spellbook.getTarget()
    else:
      av = spellbook.getInvoker()
    value = min(value, 32767)
    av.restockAllResistanceMessages(value)
...
@magicWord(category=CATEGORY_HEAD_MODERATION, types=[int])
...
@magicWord(category=CATEGORY_HEAD_MODERATION, types=[str, int, int])
...
def catalog():
    ...
    spellbook.getTarget().d_setSystemMessage(0, "You have been awarded the next catalog by a staff member!")
    return 'You now have the next catalog'
...
def gift(part, item):
    ...
    if part.startswith("clo"):
      if item in CatalogClothingItem.ClothingTypes:
        item = CatalogClothingItem.CatalogClothingItem(item, 0)
      else:
        return "That clothing item does not exist"

    if part.startswith("acc"):
      if item in CatalogAccessoryItemGlobals.AccessoryTypes:
        item = CatalogAccessoryItem.CatalogAccessoryItem(item)
      else:
        return "That accessory item does not exist"

    if item.reachedPurchaseLimit(av):
      return "They already own one"

    item.deliveryDate = int(time.time() / 60)
    av.onOrder.append(item)
    av.b_setDeliverySchedule(av.onOrder)
    av.d_setSystemMessage(0, "You have been awarded the %s by a staff member!" % (item.getName()))

    return "%s:%s in the mail" % (item.getTypeName(), item.getName())

@magicWord(category=CATEGORY_HEAD_MODERATION)
def allplayers():
    out = "\n\nCMD\n"

    from toontown.toon.DistributedNPCToonBaseAI import DistributedNPCToonBaseAI
    for doId, obj in spellbook.getTarget().air.doId2do.items():
        if obj.__class__ == DistributedToonAI:
            if not isinstance(obj, DistributedNPCToonBaseAI):
...
def fanfare():
    ...
    return "Can you hear it Surlee? The excitement."
...
@magicWord(category=CATEGORY_PROGRAMMER)
def v2():
    v2 = simbase.air.suitInvasionManager.startInvasion(2, 2, 1)
    return v2

@magicWord(category=CATEGORY_COMMUNITY_MANAGER)
def waiter():
    #This actually spawns V2s
    waiter = simbase.air.suitInvasionManager.startInvasion(2, 2, 2)
    return waiter

@magicWord(category=CATEGORY_PROGRAMMER, types=[int, int, int, int, int])
def invasion(suitDept, suitIndex=None, isSkelecog=0, isV2=0, isWaiter=0):
    flags = [isSkelecog, isV2, isWaiter]
    for flag in flags:
...
def sbhq():
    ...
    """
    av = spellbook.getTarget()
    av.addTeleportAccess(ToontownGlobals.SellbotHQ)
    av.d_setSystemMessage(0, "Lord Lowden Clear: Congratulations fellow ranger! Your hard work has granted you teleportation access to Sellbot Headquarters!")
    return "Gave %s access to Sellbot Headquarters." % (av.getName())
...
def tickets(tickets):
    ...
    """
    if not 0 <= tickets <= 5000:
        return 'Racing tickets value must be in range (0-5000).'
    invoker = spellbook.getTarget()
    invoker.b_setTickets(tickets)
    invoker.d_setSystemMessage(0, 'You were given %d tickets by a Resistance Ranger!' % tickets)
    return 'Set your tickets to: %d' % tickets

@magicWord(category=CATEGORY_MODERATOR)
def unlimitedGags():
    """ At the start of each round it restocks the user's gags """
    av = spellbook.getTarget()
...
def message(message):
    target = spellbook.getTarget()
    target.d_setSystemMessage(0, message)
    return 'Send %s a message!' % target.getName()

@magicWord(category=CATEGORY_ADMINISTRATOR, types=[str])
def cloneToon(missingTrack):
    #Taken from maxToon
    invoker = spellbook.getTarget()

    target.b_setMaxHp(100)
    invoker_b_setHp(100)
    ...
    invoker.b_setTickets(1000)

    # Max their quest carry limit:
    invoker.b_setQuestCarryLimit(4)

    invoker.sendUpdate('setTeleportOverride', [1])
    invoker.b_setTeleportAccess([1000,2000,3000,4000,5000,8000,9000])
    ...
    #Steel is better. Sometimes people don't want gold? I don't know
    invoker.b_setFishingRod(3)
