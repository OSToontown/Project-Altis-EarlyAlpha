...
class RemoteAccountDB(AccountDB):
    ...
    def submitNameRequest(self, avId, name, callback, errback):
        self.csm.air.webRpc.submitNameRequest(config.GetString('general'), avId, name,
...
class LoadAvatarFSM(AvatarOperationFSM):
    ...
    def enterSetAvatar(self):
        ...
        self.csm.air.friendsManager.avId2ChatMode[self.avId] = self.account.get('CHAT_MODE', 1)
...
class ClientServicesManagerUD(DistributedObjectGlobalUD):
    ...
    def setLoginEnabled(self, enable):
        if not enable:
            self.notify.warning('All future logins are now rejected.')
    ...
    def login(self, cookie, authKey):
        ...
        if not self.loginsEnabled:
            ...
            dg.addUint16(200)
            dg.addString('Logging into the server is currently disabled by a developer is working on the game! Please Try again later.')
            self.air.send(dg)
