...
class BossCog(Avatar.Avatar):
    ...
    def getAnim(self, anim):
        ...
        if anim == None:
            ...
            ival = Sequence()
            ival = Parallel(ActorInterval(self, 'Fb_jump'), SoundInterval(self.swishSfx, duration=1.1, node=self), SoundInterval(self.boomSfx, duration=1.9)), Sequence(Wait(1.21), Func(self.announceAreaAttack))
            if self.twoFaced:
                self.happy = 0
            else:
                self.happy = 1
