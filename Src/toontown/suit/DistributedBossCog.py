...
class DistributedBossCog(DistributedAvatar.DistributedAvatar, BossCog.BossCog):
    ...
    def loseCogSuits(self, toons, battleNode, camLoc, arrayOfObjs = False):
        ...
        for toon in toonArray:
            ...
            suitsOff.append(Sequence(Func(dustCloud.reparentTo, toon), Parallel(dustCloud.track, Sequence(Wait(0.3), Func(toon.takeOffSuit), Func(toon.sadEyes), Func(toon.blinkEyes), Func(toon.play, 'slip-backward'), Wait(0.7))), Func(dustCloud.detachNode), Func(dustCloud.destroy), Wait(2.05), Func(toon.loop, 'neutral')))
