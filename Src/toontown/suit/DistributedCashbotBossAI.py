...
class DistributedCashbotBossAI(DistributedBossCogAI.DistributedBossCogAI, FSM.FSM):
    ...
    def generateSuits(self, battleNumber):
        ...
        skelecogs = self.invokeSuitPlanner(14, 1)
    ...
    def magicWordStunGoons(self):
        if self.state == 'BattleThree':
            for goon in self.goons:
                goon.request('Stunned')
...
def cfo(action):
    ...
    boss = getCFO(invoker)
    if not boss:
        return "You aren't in a CFO battle!"
    if action == "resetGoons":
        boss.magicWordResetGoons()
    elif action == "resetProps":
        boss.magicWordReset()
    elif action == "hit":
        boss.recordHit(5)
    elif action == "stunGoons":
        boss.magicWordStunGoons()
    elif action == "stopHelmets":
        boss.stopHelmets()
