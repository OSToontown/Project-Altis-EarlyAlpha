...
class GetToonDataFSM(FSM):
    ...
    database and return it back to the TTFMUD via a callback.
    """
...
class UpdateToonFieldFSM(FSM):
    ...
    database and return a callback to the TTFMUD.
    """
...
class GetFriendsListFSM(FSM):
    ...
    this may have a huge performance impact on the TTFMUD as it may
    ...
    data the TTFMUD has about a toon.
    """
...
class TTFriendsManagerUD(DistributedObjectGlobalUD):
    """
    The Toontown Friends Manager UberDOG, or TTFMUD for short.
    ...
    def __rfGotToonFields(self, success, requesterId, fields, avId=None, final=False):
        ...
        if fields['ID'] not in [requesterId, avId]:
            ...
            # to edit! DEFCON 5!
            self.notify.warning('TTFMUD.__rfGotToonFields received wrong toon fields from db, requesterId=%d' % requesterId)
            return
    ...
    def requestAvatarInfo(self, avIds):
        ...
        if not avIds:
            ...
            self.air.writeServerEvent('suspicious', avId=requesterId, issue='Sent a blank list of avIds for requestAvatarInfo in TTFMUD')
            return
