...
class TrackPoster(DirectFrame):
    ...
    def __init__(self, trackId, callback):
        ...
        self.chooseButton = DirectButton(parent=self.poster, relief=None, image=(guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale=(0.7, 1, 1), text=TTLocalizer.TrackChoiceGuiChoose, text_scale=0.06, text_pos=(0, -0.02), command=callback, extraArgs=[trackId], pos=(0, 0, 0.22), scale=0.8)
...
class TrackChoiceGui(DirectFrame):
    ...
    def __init__(self, tracks, timeout):
        ...
        self.downArrow = DirectButton(parent=self, relief=None, image=arrowImage, pos=(-0.60, 0, 0), command=self.setPage, extraArgs=[-1])
        self.upArrow = DirectButton(parent=self, relief=None, image=arrowImage, pos=(0.60, 0, 0), scale=-1, command=self.setPage, extraArgs=[1])
        gui.removeNode()
