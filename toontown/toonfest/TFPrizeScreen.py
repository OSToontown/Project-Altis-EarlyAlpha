# Embedded file name: toontown.toonfest.TFPrizeScreen
...
from toontown.catalog import CatalogGenerator, CatalogItemPanel, CatalogInvalidItem, CatalogItemTypes, CatalogItemList
...
class TFPrizeScreen(DirectFrame):
    notify = DirectNotifyGlobal.directNotify.newCategory('TFPrizeScreen')
    ...
    def __init__(self, parent = aspect2d, **kw):
        ...
        self.load()
        self.initialiseoptions(TFPrizeScreen)
    ...
    def load(self):
        ...
        self.tokenJar = DirectLabel(jarOrigin, relief=None, image=jarGui.find('**/jar'), text=str(base.localAvatar.getToonfestTokens()), text_align=TextNode.ACenter, text_scale=0.2, text_pos=(0.0, -0.1), text_fg=(0.95, 0.95, 0, 1), text_shadow=(0, 0, 0, 1), text_font=ToontownGlobals.getSignFont())
    ...
    def update(self, lock = 0):
        if hasattr(self, 'tokenJar'):
            self.tokenJar['text'] = str(base.localAvatar.getToonfestTokens())
            if lock == 0:
