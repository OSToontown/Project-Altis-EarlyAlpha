...
class AntialiasingSMAAPass(RenderPass):
    ...
    def create(self):
        ...
        self.areaTex = Globals.loader.loadTexture(
            "pipeline_data/Antialiasing/SMAA_AreaTexGL.png")
        self.searchTex = Globals.loader.loadTexture(
            "pipeline_data/Antialiasing/SMAA_SearchTexGL.png")
    ...
    def setShaders(self):
        edgeShader = Shader.load(Shader.SLGLSL, 
            "shaders/Antialiasing/SMAA/EdgeDetection.vertex", 
            "shaders/Antialiasing/SMAA/EdgeDetection.fragment")
        self._edgesBuffer.setShader(edgeShader)

        weightsShader = Shader.load(Shader.SLGLSL, 
            "shaders/Antialiasing/SMAA/BlendingWeights.vertex", 
            "shaders/Antialiasing/SMAA/BlendingWeights.fragment")
        self._blendBuffer.setShader(weightsShader)

        neighborShader = Shader.load(Shader.SLGLSL, 
            "shaders/Antialiasing/SMAA/Neighbors.vertex", 
            "shaders/Antialiasing/SMAA/Neighbors.fragment")

        for buff in self._neighborBuffers:
            buff.setShader(neighborShader)

        resolveShader = Shader.load(Shader.SLGLSL, 
            "shaders/Antialiasing/SMAA/Resolve.vertex", 
            "shaders/Antialiasing/SMAA/Resolve.fragment")
