...
from panda3d.core import Filename, StreamReader, VirtualFileSystem, Texture, PNMImage, SamplerState
...
class IESLoader(DebugObject):
    ...
    def loadIESProfiles(self, directory):
        ...
        files = VirtualFileSystem.getGlobalPtr().scanDirectory(Filename(directory))

        for entry in files.getFiles():
            entry = str(entry.getFilename())
            if str(entry).lower().endswith(".ies"):
                combinedPath = join(directory, entry)
    ...
    def _loadIESProfile(self, name, filename):
        ...
        # Open the IES file
        content = StreamReader(VirtualFileSystem.getGlobalPtr().openReadFile(filename, 1), 1).getString()
