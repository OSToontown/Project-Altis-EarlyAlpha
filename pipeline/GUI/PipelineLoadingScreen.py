...
class PipelineLoadingScreen:
    ...
    def render(self):
        ...
        self.font = loader.loadFont("pipeline_data/Font/Multicolore.otf")

        xOffs = (w - 1920) / 2.0
        yOffs = (h - 1080) / 2.0

        self.bgImage = BetterOnscreenImage(image="pipeline_data/GUI/LoadingScreen.png", parent=self.node, w=1920, h=1080, x=xOffs, y=yOffs)

        cr = 212

        self.circlePart1 = BetterOnscreenImage(image="pipeline_data/GUI/LoadingCirclePart.png", parent=self.node, w=cr, h=cr, x=(w-cr) / 2, y = (h-cr) / 2 - 50)
        self.circlePart2 = BetterOnscreenImage(image="pipeline_data/GUI/LoadingCirclePart.png", parent=self.node, w=cr, h=cr, x=(w-cr) / 2, y = (h-cr) / 2 - 50)
        self.circlePart3 = BetterOnscreenImage(image="pipeline_data/GUI/LoadingCirclePart.png", parent=self.node, w=cr, h=cr, x=(w-cr) / 2, y = (h-cr) / 2 - 50)
        self.circlePart4 = BetterOnscreenImage(image="pipeline_data/GUI/LoadingCirclePart.png", parent=self.node, w=cr, h=cr, x=(w-cr) / 2, y = (h-cr) / 2 - 50)

        self.circleBg = BetterOnscreenImage(image="pipeline_data/GUI/LoadingCircleBg.png", parent=self.node, w=180, h=180, x=(w-180) / 2, y = (h-180) / 2 - 50)

        self.logoImage = BetterOnscreenImage(image="pipeline_data/GUI/RPIcon.png", parent=self.node, w=316, h=316, x=(w - 316) /2, y = (h-316) / 2 - 50)
